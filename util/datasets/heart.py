...
class Heart(BaseDataset):
    ...
    BASE_DIR, NUM_CLASS, CROP_SIZE = ('Task02_Heart/', 2, 256)
    IN_CHANNELS = 1
    CLASS_WEIGHTS = None
    mean = [0.3949544]
    std = [0.41724333]
    ...
    def __init__(self, root, split='train', mode=None):
        super(Heart, self).__init__(root, split, mode, norm={'mu': self.mean, 'std': self.std})
            ...
            for root, dirs, files in os.walk(image_path):
                ...
                for file in files:
                    if '.nii.gz' in file:
                        continue
                    # image
                    # '../train_tiny_data/imgseg/Task02_Heart/imagesTr/la_003/0.png'
                    image_dir = os.path.join(image_path, path[-1], file)
                    # mask
                    # '../train_tiny_data/imgseg/Task02_Heart/labelsTr/la_003/0.png'
